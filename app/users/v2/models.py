...
class User:
    def add_user(self, email, password,username, firstname, lastname, phonenumber):
        ...
        retrieved_username = ReportDB.retrieve_all(user_query)
        retrieved_email = ReportDB.retrieve_all(email_query)

        if retrieved_username or retrieved_email:
            return False    
        user_query = """INSERT INTO users (email, password,username,firstname,lastname,phonenumber) VALUES (%s, %s,%s,%s,%s,%s) RETURNING username, id"""
        tupl = (email, hash_password)
    ...
    def get_user_by_email(self, email):
        email_query = """SELECT * FROM users WHERE email = '{}'""".format(email)
        repo = ReportDB.retrieve_all(email_query)
        if not repo:
            return False
        return repo
    ...
    def get_user_by_username(self, username):
        username_query = """SELECT * FROM users WHERE username = '{}'""".format(username)
        repo = ReportDB.retrieve_all(username_query)
        if not repo:
            return False
        return repo
    ...
    def validate_password(self, password, email):
        ...
        result = ReportDB.retrieve_one(query)
